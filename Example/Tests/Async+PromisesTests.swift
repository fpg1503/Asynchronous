...
class AsyncPromisesTests: XCTestCase {
    ...
    func testSynchronousResolve() {
        ...
        wait { expectation in
            (async.promise() as PromisesPromise).then { actual in
    ...
    func testSynchronousReject() {
        ...
        wait { expectation in
            (async.promise() as PromisesPromise).then { value in
    ...
    //MARK: - BrightFutures -> Promises
    func testPromisesConversion_AsyncSucceeds() {
        let expected = 3
    ...
    func testPromisesConversion_AsyncFails() {
        let expected = TestError.somethingReallyHorribleHappened
        let promise = Promise<Int> { resolve, reject in
            DispatchQueue.global(qos: .background).asyncAfter(deadline: .now() + TestHelper.delayTime) {
